...
class InstamojoAPI {
    ...
    */
    public function __construct($api_key, $auth_token=null, $endpoint)
    {
        $this->api_key = (string) $api_key;
        $this->auth_token = (string) $auth_token;
        if($endpoint == 'sandbox'){
            $this->endpoint = 'https://test.instamojo.com/api/1.1/' ;
        }
        else{
    ...
    */  
    ...
    public function generatePaymentUrl(array $payment_request_data) {
      $response = $this->api_call('POST', 'payment-requests', $payment_request_data);
